...
add_action( 'cmb2_init', 'portfolio_custom_box' );

function portfolio_custom_box() {
...
function get_portfolio(){
    ...
    foreach($items as $item){
		$year = get_post_meta( $item->ID, 'portfolio_year', true ) ? get_post_meta( $item->ID, 'portfolio_year', true ) : false;
        $output .='<li class="portfolio">';
        $output .=' <div class="portfolio-name">';
		$output .='<span class="glyphicon glyphicon-paperclip"></span>';
        $output .=      $item->post_title;
        $output .=' </div>';
        $output .='<div class="portfolio-level">';
